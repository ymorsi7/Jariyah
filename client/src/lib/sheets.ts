...
import type { Charity, Donation, UserProfile, Badge } from './types';
...
export const sheetsService = {
  ...
  async getUserProfile(userId: string): Promise<UserProfile | null> {
    ...
    try {
      ...
      const headers = rows[0];
      const users = rows.slice(1).map(row => rowToObject<UserProfile & { id: string }>(headers, row));
    ...
    try {
      ...
      if (!existingProfile) {
        ...
        const rowIndex = rows.findIndex(row => 
          rowToObject<{ id: string }>(headers, row).id === userId
        );
